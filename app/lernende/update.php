...
$email_privat = $data["email_privat"] ?? null;
$birthdate = $data["birthdate"] ?? null;

if (!$id || !$vorname || !$nachname || !$strasse || !$plz || !$ort || !$nr_land || !$geschlecht || !$telefon || !$handy || !$email || !$email_privat || !$birthdate) {
    echo json_encode([
        "status" => "error",
...
$db = DB::getPdo();
$statement = $db->prepare("UPDATE tbl_lernende SET vorname = :vorname, nachname = :nachname, strasse = :strasse, plz = :plz, ort = :ort, nr_land = :nr_land, geschlecht = :geschlecht, telefon = :telefon, handy = :handy, email = :email, email_privat = :email_privat, birthdate = :birthdate WHERE id_lernende = :id");
